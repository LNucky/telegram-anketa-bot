...
from db.models import Form, User
...
class FormRepository(BaseRepository[Form]):
    ...
    async def get_by_id_with_author(self, form_id: int) -> Optional[Form]:
        """Получить форму по ID с загруженным автором."""
    ...
    async def get_all_by_author_id(self, author_id: int) -> list[Form]:
        """Получить все формы пользователя по его ID."""
        result = await self.session.execute(
            select(Form).where(Form.author_id == author_id)
        )
        return list(result.scalars().all())

    async def get_all_with_authors(self) -> list[Form]:
        """Получить все формы с загруженными авторами."""
        result = await self.session.execute(
            select(Form).options(selectinload(Form.author))
        )
        return list(result.scalars().all())
